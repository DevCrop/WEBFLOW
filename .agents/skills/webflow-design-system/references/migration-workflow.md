...
```text
root     intro-title; bind one approved inherited color variable per variant
eyebrow  section-head-eyebrow + medium
title    section-head-title + regular
...
root     section-title; bind one approved inherited color variable per variant
eyebrow  section-head-eyebrow + medium
...
Reusable component internals omit `text-*`. Base and invert variants switch only the outermost owned root color; nested independently themed controls retain their own color ownership.
...
Replace both instances with the existing `intro-title` component. Map `heading-64 + regular + text-title-v2` to the component-internal `section-head-title + regular`; the `intro-title` root owns inherited color and every semantic selector binds `Font/Base`. The component and `text-title-v2` may move to `zero-use` only after both replacement instances and every variant/custom-code reference are verified.
